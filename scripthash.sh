...
P2PKH="76A914"
P2PKH="${P2PKH}${decode:2:-8}"
P2PKH="${P2PKH}88AC"

echo ${P2PKH} | xxd -r -p | sha256sum -b | awk '{print $1}' | tac -rs .. | xargs echo

#echo -n ${P2PKH}| tr '[:upper:]' '[:lower:]' | xxd -r -p | sha256sum -b | awk '{print $1}' | tac -rs .. | xargs echo
#echo -n ${P2PKH}| tr '[:upper:]' '[:lower:]'
